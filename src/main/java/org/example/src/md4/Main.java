...
public class Main {
    ...
    static int sumMod32(int a,long b){
        return (int)((a + b) % Math.pow(2,32));
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            F = (long)stack.get(1) ^ (long)stack.get(2) ^ (long)stack.get(3);
            //F = F >>> 32;
            /*if (F < 0) {
                System.out.println("before F = " + F + " I = " + i); F = Math.abs(F) + (int)Math.pow(2,31);
                System.out.println("after F = "+F);}*/

            FileWork.reportWrite(report_file,"\t1) F = " + F + " = " + Integer.toBinaryString((int)F));
            //System.out.println("\t1) F = " + Integer.toBinaryString(F));

            temp = sumMod32(stack.get(0),F);
            FileWork.reportWrite(report_file,"\t2) (A + F) mod 2 ^ 32 = " + stack.get(0) + " + " + F + " = " + temp + " = " + Integer.toBinaryString(stack.get(0)) + " + " + Integer.toBinaryString((int)F) + " = " + Integer.toBinaryString(temp));
            //System.out.println("\t2) (A + F) mod 2 ^ 32 = " + Integer.toBinaryString(temp));

            temp = sumMod32(temp,i);
            FileWork.reportWrite(report_file,"\t3)P(2) + Mi mod 2 ^ 32 = " + (temp - i) + " + " + i + " = " + Integer.toBinaryString(temp));
